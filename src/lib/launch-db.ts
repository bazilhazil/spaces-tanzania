...
export async function fetchLaunchReport(): Promise<LaunchReport> {
  const [
    ...
    count("properties", same),
    count("property_media", same),
    count("leads", same),
    count("bookings", same),
    count("deals", same),
    count("reviews", same),
    count("notifications", same),
    count("profiles", same),
    count("payments", (q) => (q as never as { in: (a: string, b: string[]) => unknown }).in("status", ["paid", "succeeded"])),
    count("properties", (q) => (q as never as { eq: (a: string, b: string) => unknown }).eq("status", "pending")),
    count("verification_requests", (q) => (q as never as { eq: (a: string, b: string) => unknown }).eq("status", "pending")),
    count("bookings", (q) => (q as never as { eq: (a: string, b: string) => unknown }).eq("status", "pending")),
    count("deals", (q) => (q as never as { not: (a: string, b: string, c: string) => unknown }).not("stage", "in", "(completed,cancelled)")),
    count("payments", (q) => (q as never as { eq: (a: string, b: string) => unknown }).eq("status", "failed")),
    count("payments", (q) => (q as never as { in: (a: string, b: string[]) => unknown }).in("status", ["pending", "processing"])),
    count("safety_reports", (q) => (q as never as { in: (a: string, b: string[]) => unknown }).in("status", ["new", "under_review", "more_info"])),
    count("properties", (q) => (q as never as { eq: (a: string, b: boolean) => unknown }).eq("under_review", true)),
    count("reviews", (q) => (q as never as { eq: (a: string, b: string) => unknown }).eq("status", "pending")),
